...
use hypertok::load_tokenizer::htk::{
    PREBUILT_COMPACT_REPLAY_SECTION_ID, build_prebuilt_compact_replay_image, load_htk_slice,
};
...
fn with_compact_replay(source: &[u8]) -> Vec<u8> {
...
#[test]
fn compact_replay_preserves_runtime_behavior() {
    let source = tracked("hypertok-vocab/gpt2/vocab.htk");
    let candidate = with_compact_replay(&source);
    let mut reference = load_htk_slice(&source).expect("load source vocabulary");
    let mut compact = load_htk_slice(&candidate).expect("load compact replay vocabulary");
    for text in [
        ...
        let actual = compact.tokenizer.encode(text);
        assert_eq!(actual, expected, "encode parity for {text:?}");
        assert_eq!(compact.tokenizer.decode(&actual), text.as_bytes());
    }
}

#[test]
fn digest_valid_compact_replay_corruption_is_refused() {
    let source = tracked("hypertok-vocab/gpt2/vocab.htk");
    let mut candidate = with_compact_replay(&source);
    let entry = *ValidatedFile::read(&candidate)
        .expect("validate candidate")
        .section_entry(PREBUILT_COMPACT_REPLAY_SECTION_ID)
        .expect("compact replay section");
    ...
    assert!(
        ...
        "digest-valid compact replay corruption must remain on the untrusted refusal path"
    );
}
