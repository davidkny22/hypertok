...
use hypertok::load_tokenizer::htk::{PREBUILT_PAIR_RANKS_SECTION_ID, build_prebuilt_pair_image};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.next().is_some() {
        return Err("usage: hypertok-prebuilt-fixture <input.htk> <output.htk>".into());
    }

    let source = fs::read(&input)?;
    let file = ValidatedFile::read(&source)?;
    let image = build_prebuilt_pair_image(&source)?;
    let mut sections = Vec::with_capacity(file.sections().len() + 1);
    for entry in file.sections() {
        if entry.id == PREBUILT_PAIR_RANKS_SECTION_ID {
            continue;
        }
        let bytes = file
            .section(entry.id)
    ...
    sections.push(Section::extension(PREBUILT_PAIR_RANKS_SECTION_ID, image)?);
