...
use hypertok::load_tokenizer::htk::{
    PREBUILT_COMPACT_REPLAY_SECTION_ID, build_prebuilt_compact_replay_image,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.next().is_some() {
        return Err("usage: hypertok-compact-replay-fixture <input.htk> <output.htk>".into());
    }

    let source = fs::read(&input)?;
    let file = ValidatedFile::read(&source)?;
    let image = build_prebuilt_compact_replay_image(&source)?;
    let mut sections = Vec::with_capacity(file.sections().len() + 1);
    for entry in file.sections() {
        if entry.id == PREBUILT_COMPACT_REPLAY_SECTION_ID {
            continue;
        }
        let bytes = file
            .section(entry.id)
