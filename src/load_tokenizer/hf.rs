...
fn parse_tokenizer_json(data: &[u8]) -> Result<TokenizerJson> {
    if let Ok(ModelTypeProbe { model: Some(m) }) = super::json::from_slice::<ModelTypeProbe>(data) {
        ...
        if let Some(family) = family {
            return Err(eyre::eyre!(
                "Unsupported model type \"{family}\": hypertok supports BPE tokenizers \
    ...
    super::json::from_slice(data).map_err(|e| eyre::eyre!("Failed to parse tokenizer JSON: {e}"))
}
...
pub fn load_hf_slice(data: &[u8]) -> Result<HfTokenizer> {
    ...
    if tj.model.byte_fallback {
        ...
        {
            Err(eyre::eyre!(
                "SentencePiece BPE support is not enabled in this build"
            ))
        }
    } else {
...
fn build_sentencepiece(tj: &TokenizerJson) -> Result<SentencePieceBPE> {
    ...
    let vocab: Vec<TokenBytes> = vocab
        .into_iter()
        .map(|bytes| bytes.as_ref().to_vec().into())
        .collect();
    #[cfg(feature = "opt-resident-diet")]
    let vocab_inv: HashMap<TokenBytes, TokenId, FxBuildHasher> = vocab_inv
        .into_iter()
        .map(|(bytes, id)| (bytes.as_ref().to_vec().into(), id))
        .collect();
...
fn build_bpe(tj: &TokenizerJson) -> Result<bpe::tiktoken::Tokenizer> {
    ...
    let normalize_nfc = detect_nfc_normalizer(&tj.normalizer)?;
    tokenizer.set_normalize_nfc(normalize_nfc);
...
mod tests {
    ...
    fn tinyllama_path() -> Option<std::path::PathBuf> {
        ...
        let candidates = [
            manifest.join("results/sources/tinyllama-tokenizer.json"),
            manifest.join("../../results/sources/tinyllama-tokenizer.json"),
        ];
        let path = candidates.into_iter().find(|candidate| candidate.is_file());
        if path.is_none() {
            eprintln!("Skipping: pinned TinyLlama tokenizer source is unavailable");
        }
        path
    }
