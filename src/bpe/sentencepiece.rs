use crate::bpe::{TokenBytes, bpe_merge_symbols_ranked};
...
impl PrecompiledCharsmap {
    ...
    pub(crate) fn normalize_into(&self, input: &str, out: &mut String) {
        ...
        let mut i = 0usize;
        while i < bytes.len() {
            // Skip to the next attention byte (non-ASCII, control, or DEL).
            let mut j = i;
...
pub struct SentencePieceBPE {
    ...
    pub(crate) vocab: Vec<TokenBytes>,
    /// Maps byte sequences → token IDs (for character lookup).
    pub(crate) vocab_inv: HashMap<TokenBytes, TokenId, FxBuildHasher>,
...
impl SentencePieceBPE {
            ...
            match op {
                NormOp::Nfc => {
                    if !crate::bpe::nfc::is_normalized(&s) {
                        ...
                        crate::bpe::nfc::normalize_to(&s, &mut out);
                        s = Cow::Owned(out);
                    }
                }
    ...
    fn added_token_cut_blocks(&self, text: &str) -> Vec<std::ops::Range<usize>> {
        ...
        let collect = |spec: &AddedTokenSpec| {
            let content = spec.content.as_bytes();
            let interior = content.contains(&b' ') || content.contains(&0xE2);
            if content.is_empty() || !(interior || unguarded || spec.lstrip || spec.rstrip) {
                return Vec::new();
            }
            let content = spec.content.as_bytes();
        ...
        #[cfg(feature = "threaded-wasm")]
        let mut blocks: Vec<std::ops::Range<usize>> = {
            use rayon::prelude::*;
            self.added_tokens
                .par_iter()
        ...
        #[cfg(not(feature = "threaded-wasm"))]
        let mut blocks: Vec<std::ops::Range<usize>> =
            self.added_tokens.iter().flat_map(collect).collect();
...
impl SentencePieceBPE {
    ...
    fn encode_units_impl<const RAW: bool, F: FnMut(&[TokenId])>(
        ...
        while block < bytes.len() {
            let mut mask = self.candidate_mask::<RAW>(bytes, block);
    ...
    #[inline]
    fn candidate_mask<const RAW: bool>(&self, bytes: &[u8], block: usize) -> u32 {
        self.candidate_mask_scalar::<RAW>(bytes, block)
    }

    #[inline]
    fn candidate_mask_scalar<const RAW: bool>(&self, bytes: &[u8], block: usize) -> u32 {
        let mut mask = 0;
        for (index, &byte) in bytes[block..bytes.len().min(block + 32)].iter().enumerate() {
            if byte == 0xE2 || (RAW && byte == b' ') || self.split_bytes.contains(&byte) {
                mask |= 1 << index;
            }
        }
        mask
