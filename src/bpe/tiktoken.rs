...
use crate::bpe::{
    ByteRemapping, MergeScratch, PairRankTable, SHORT_MERGE_MAX, TokenBytes,
...
use crate::bpe::{bpe_merge_symbols_ranked_by, bpe_merge_symbols_ranked_slice_by};
...
#[cfg(feature = "profiling")]
use crate::profiling::{
    ProfileAccumulator, ProfileClock, ProfileSnapshot, RankCounters, sample_profile_batch,
};
...
pub struct Tokenizer {
    ...
    pub(crate) vocab: Arc<Vec<TokenBytes>>,
    pub(crate) vocab_inv: Arc<HashMap<TokenBytes, TokenId, rustc_hash::FxBuildHasher>>,
    ...
    #[cfg(feature = "opt-miss-path-diet")]
    pretoken_cache_long: LongPretokenCache,
    ...
    /// Apply the ByteLevel post-process offset trim when deriving `starts`.
    /// This changes no token IDs and therefore stays out of the encode path.
    trim_offsets: bool,
...
fn nfc_segment<'a>(seg: &'a [u8], buf: &'a mut String) -> &'a [u8] {
    ...
    if crate::bpe::nfc::is_normalized(s) {
        return seg;
    }
    crate::bpe::nfc::normalize_to(s, buf);
    buf.as_bytes()
}
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NormalizationUnavailable => {
                formatter.write_str("NFC source mapping is unavailable in this build")
            }
            Self::UnknownTokenId(id) => write!(formatter, "unknown token id {id}"),
            Self::EmptyToken(id) => write!(formatter, "token id {id} has no bytes"),
            Self::TokenBytesMismatch {
                token_index,
                byte_offset,
            } => write!(
                formatter,
                "token bytes diverge from transformed input at token {token_index}, byte {byte_offset}"
            ),
            Self::TransformedLengthMismatch { consumed, actual } => write!(
                formatter,
                "token bytes consume {consumed} transformed bytes, expected {actual}"
            ),
        }
    }
}
...
impl From<SourceMapError> for StartError {
    fn from(error: SourceMapError) -> Self {
        match error {
            SourceMapError::InvalidUtf8 => Self::InvalidUtf8,
            SourceMapError::OffsetOverflow => Self::OffsetOverflow,
        }
    }
}

#[cfg(feature = "opt-chunk-prescan")]
pub(crate) enum AddedPiece<'a> {
    Segment(&'a [u8]),
    Added(u32),
}
...
fn apply_added_token_overwrites(
    ...
    vocab_inv: &HashMap<TokenBytes, TokenId, rustc_hash::FxBuildHasher>,
    ...
    for tok in added_tokens {
        ...
        let Some(&id) = vocab_inv.get(content.as_ref()) else {
            continue;
        };
...
impl Tokenizer {
    ...
    #[cfg(all(feature = "opt-fused-pair-ranks", feature = "opt-resident-diet"))]
    pub(crate) fn new_pair_ranks_token_views(
        pair_ranks: PairRankTable,
        vocab: Vec<TokenBytes>,
        byte_remapping: Option<ByteRemapping>,
    ) -> Self {
    ...
    fn from_pair_rank_table(
        ...
        let vocab_inv: HashMap<TokenBytes, TokenId, rustc_hash::FxBuildHasher> =
            crate::cold_construction::measure("inverse-vocabulary", || {
                vocab
                    .iter()
                    .cloned()
        ...
        let pretoken_cache = crate::cold_construction::measure("pretoken-cache-seed", || {
            ShortPretokenCache::with_at_least(0, 0)
        });
        Tokenizer {
    ...
    #[cfg(feature = "opt-resident-diet")]
    pub(crate) fn new_ranked_token_views(
        ranked_merges: RankedMerges,
        vocab: Vec<TokenBytes>,
        byte_remapping: Option<ByteRemapping>,
    ) -> Self {
    ...
    fn from_tables(
        ...
        let pair_ranks = crate::cold_construction::measure("core-pair-table", || {
            if ranked_merges.is_none() {
    ...
    /// miss.
    /// With `opt-cold-diet`, construction allocates an empty table at the
    /// requested floor and lets the ordinary miss path populate it lazily.
    fn seeded_pretoken_cache(
        ...
        #[cfg(feature = "opt-cold-diet")]
        {
            ...
            return ShortPretokenCache::with_at_least(0, min_slots);
        }
    ...
    pub fn from_ranks(vocab: Vec<Vec<u8>>) -> Result<Self> {
        ...
        let vocab = vocab
            .into_iter()
            .map(Into::into)
            .collect::<Vec<TokenBytes>>();
        let vocab_inv: HashMap<TokenBytes, TokenId, rustc_hash::FxBuildHasher> = vocab
            .iter()
            .cloned()
            .zip((0..).map(TokenId::from))
            .collect();
    ...
    pub(crate) fn fork_sized(&self, expected_bytes: usize) -> Self {
        ...
        Tokenizer {
            ...
            pretoken_cache_long: {
                #[cfg(not(feature = "opt-miss-path-diet"))]
                {
                    HashMap::with_capacity_and_hasher(long_cap, rustc_hash::FxBuildHasher {})
                }
                #[cfg(feature = "opt-miss-path-diet")]
                {
                    LongPretokenCache::with_capacity(long_cap)
                }
            },
            merge_scratch: MergeScratch::default(),
            symbol_scratch: Vec::new(),
    ...
    /// Retain the ByteLevel `trim_offsets` flag for the detailed offset path.
    pub fn set_trim_offsets(&mut self, trim_offsets: bool) {
    ...
    /// the merge loop.
    ///
    /// Normally rewrites the vocab-seeded short-cache entries to the new
    /// flag's seed values and reasserts added-token overwrites. With
    /// `opt-cold-diet`, the cache starts empty, so the ordinary miss path
    /// observes the new flag and populates entries lazily.
    ///
    ...
    pub fn set_added_tokens(&mut self, added_tokens: Vec<AddedTokenDef>) {
        ...
        for tok in &outgoing {
            let content = &tok.content;
            if !(1..=15).contains(&content.len()) || self.vocab_inv.get(content.as_ref()).is_none()
            {
                continue;
            }
            let key = pack_pretoken_key(content).expect("length checked <= 15");
    ...
    pub fn add_special_tokens(&mut self, tokens: impl IntoIterator<Item = (Vec<u8>, TokenId)>) {
        ...
        for (content, id) in tokens {
            ...
            if idx >= vocab.len() {
                vocab.resize(idx + 1, Vec::new().into());
            }
    ...
    pub fn merge_entries(&self) -> Vec<(&[u8], &[u8])> {
        let mut ranked: Vec<(u32, u32, u32)> = match self.ranked_merges.as_deref() {
            Some(rm) => rm
                ...
                .collect(),
            None if !self.merges.is_empty() => self
                .merges
                .iter()
            ...
            None => self
                ...
                .as_deref()
                .into_iter()
                .flat_map(PairRankTable::entries)
                .map(|(a, b, m)| (a.0, b.0, m.0))
                .collect(),
        };
    ...
    /// Release the id-ranked construction map after the complete pair table
    /// and loader-time caches have been built.
    ///
    /// Returns false for explicit-rank models and for models whose IDs could
    /// not be represented by `PairRankTable`; those still need their maps.
    #[cfg(feature = "opt-resident-diet")]
    pub(crate) fn discard_redundant_id_merges(&mut self) -> bool {
        if self.ranked_merges.is_some() || self.pair_ranks.is_none() {
            return false;
        }
        self.merges = Arc::new(HashMap::with_hasher(rustc_hash::FxBuildHasher {}));
        true
    }

    /// Pull one chunk of prescanned spans through the tokenizer's ordinary
    /// keyed batch fill so scanner features remain reachable from streaming
    /// chunked encoding.
    #[cfg(all(feature = "opt-chunk-prescan", feature = "opt-scan-two-phase"))]
    pub(crate) fn fill_chunk_stream_batch<'i>(
        &mut self,
        pretokens: &mut impl PretokenSpans<'i>,
        batch: &mut SpanBatch<'i>,
    ) -> usize {
        let cache = &self.pretoken_cache;
        pretokens.fill_spans_keyed(batch, &|hash| cache.prefetch_l2(hash))
    }

    /// Emit one already-filled streaming chunk through the shared cache
    /// probe and output loop.
    #[cfg(all(feature = "opt-chunk-prescan", feature = "opt-scan-two-phase"))]
    pub(crate) fn emit_chunk_stream_batch(
        &mut self,
        batch: &SpanBatch<'_>,
        count: usize,
        output: &mut Vec<u32>,
    ) {
        self.probe_emit_chunk(batch, count, output, |_, _| {});
    }
    ...
    fn for_each_piece(&mut self, bytes: &[u8], mut f: impl FnMut(&mut Self, Piece<'_>)) {
        ...
        while pos < bytes.len() {
            ...
            match added {
                Some((start, end, id, _, rstrip)) => {
                    f(self, Piece::Added { id, start, end });
    ...
    #[cfg(feature = "opt-chunk-prescan")]
    pub(crate) fn try_for_each_added_piece<E>(
        &mut self,
        bytes: &[u8],
        mut visit: impl FnMut(&mut Self, AddedPiece<'_>) -> Result<(), E>,
    ) -> Result<(), E> {
        let mut failure = None;
        self.for_each_piece(bytes, |this, piece| {
            if failure.is_some() {
                return;
            }
            let piece = match piece {
                Piece::Segment { encoded, .. } => AddedPiece::Segment(encoded),
                Piece::Added { id, .. } => AddedPiece::Added(id.0),
            };
            failure = visit(this, piece).err();
        });
    ...
    #[cfg(feature = "profiling")]
    #[inline(never)]
    fn for_each_piece_profiled(
        &mut self,
        bytes: &[u8],
        clock: &ProfileClock,
        mut f: impl FnMut(&mut Self, Piece<'_>),
    ) -> f64 {
        let normalize_nfc = self.normalize_nfc;
        let mut nfc_buf = String::new();
        let mut prefix_buf = Vec::new();
        let mut reserved_scan_ms = 0.0;
        let mut pos = 0;
        while pos < bytes.len() {
            let scan_started = clock.now_ms();
            let (mut seg_end, added) = match self.find_added_token(bytes, pos) {
                Some((start, end, idx)) => {
                    let token = &self.added_tokens[idx];
                    (
                        start,
                        Some((start, end, token.id, token.lstrip, token.rstrip)),
                    )
                }
                None => (bytes.len(), None),
            };
            ...
            reserved_scan_ms += clock.now_ms() - scan_started;
            f(
                self,
                Piece::Segment {
                    encoded: segment,
            ...
            match added {
                Some((start, end, id, _, rstrip)) => {
                    ...
                    reserved_scan_ms += clock.now_ms() - tail_started;
                }
                None => {
                    reserved_scan_ms += clock.now_ms() - tail_started;
                    break;
    ...
    pub fn encode_with_added_tokens(&mut self, bytes: &[u8], mut f: impl FnMut(&[TokenId])) {
        ...
        self.for_each_piece(bytes, |this, piece| match piece {
            Piece::Segment { encoded, .. } => this.memoized_encode(pt.pretokenize(encoded), &mut f),
            Piece::Added { id, .. } => f(&[id]),
        });
    }
    ...
    pub fn encode_with_added_tokens_flat(&mut self, bytes: &[u8], out: &mut Vec<u32>) {
        let pt = self.pretokenizer_type;
        self.for_each_piece(bytes, |this, piece| match piece {
            Piece::Segment { encoded, .. } => {
                this.memoized_encode_flat(pt.pretokenize(encoded), out)
            }
    ...
    #[cfg(feature = "profiling")]
    pub fn fill_span_ranges(&mut self, bytes: &[u8], out: &mut Vec<u32>) {
        let mut pretokens = self.pretokenizer_type.pretokenize(bytes);
        let mut batch = SpanBatch::new();
        let base = bytes.as_ptr() as usize;
        loop {
            let n = pretokens.fill_spans_keyed(&mut batch, &|_| {});
            for entry in &batch.entries[..n] {
                let start = entry.ptr as usize - base;
                out.push(start as u32);
                out.push((start + entry.span_len()) as u32);
            }
            if n < PRETOKEN_CHUNK {
                break;
            }
    ...
    /// Map a committed token stream back to original-input UTF-8 byte starts.
    ///
    /// The stream may come from unsplit encoding or overlap reconciliation.
    /// Exact token bytes are checked against the transformed input before any
    /// start is returned, so a stale or unrelated ID sequence is refused.
    pub fn token_starts(&mut self, input: &[u8], ids: &[u32]) -> Result<Vec<u32>, StartError> {
        if let Some(starts) = self.identity_token_starts(input, ids)? {
            return Ok(starts);
        }
        let mapped = self.mapped_input(input)?;
        let mut starts = Vec::with_capacity(ids.len());
        let mut cursor = 0_usize;
        for (token_index, &id) in ids.iter().enumerate() {
            let token = self
                .vocab
                .get(id as usize)
                .map(AsRef::as_ref)
                .ok_or(StartError::UnknownTokenId(id))?;
            if token.is_empty() {
                return Err(StartError::EmptyToken(id));
            }
            let end =
                cursor
                    .checked_add(token.len())
                    .ok_or(StartError::TransformedLengthMismatch {
                        consumed: usize::MAX,
                        actual: mapped.bytes().len(),
                    })?;
            if mapped.bytes().get(cursor..end) != Some(token) {
                return Err(StartError::TokenBytesMismatch {
                    token_index,
                    byte_offset: cursor,
                });
            }
            let leading_spaces = token.iter().take_while(|byte| **byte == b' ').count();
            let preserve_prefix = token_index == 0 && self.add_prefix_space && leading_spaces == 1;
            let start = if self.trim_offsets && leading_spaces != 0 && !preserve_prefix {
                if leading_spaces < token.len() {
                    mapped.spans()[cursor + leading_spaces].start
                } else {
                    mapped.spans()[end - 1].end
                }
            } else {
                mapped.spans()[cursor].start
            };
            starts.push(start);
            cursor = end;
        }
        if cursor != mapped.bytes().len() {
            return Err(StartError::TransformedLengthMismatch {
                consumed: cursor,
                actual: mapped.bytes().len(),
            });
        }
    ...
    fn identity_token_starts(
        &self,
        input: &[u8],
        ids: &[u32],
    ) -> Result<Option<Vec<u32>>, StartError> {
        if self.normalize_nfc && std::str::from_utf8(input).is_err() {
            return Ok(None);
        }
        let mut starts = Vec::with_capacity(ids.len());
        let mut cursor = 0_usize;
        for (token_index, &id) in ids.iter().enumerate() {
            ...
            let Some(end) = cursor.checked_add(token.len()) else {
                return Ok(None);
            };
            if input.get(cursor..end) != Some(token) {
                return Ok(None);
            }
            let leading_spaces = token.iter().take_while(|byte| **byte == b' ').count();
            let preserve_prefix = token_index == 0 && self.add_prefix_space && leading_spaces == 1;
            ...
            starts.push(u32::try_from(start).map_err(|_| StartError::OffsetOverflow)?);
            cursor = end;
        }
        Ok((cursor == input.len()).then_some(starts))
    }

    fn mapped_input(&mut self, input: &[u8]) -> Result<MappedBytes, StartError> {
        let mut output = MappedBytes::empty();
        let mut failure = None;
        self.for_each_piece(input, |this, piece| {
            if failure.is_some() {
                return;
            }
            let mapped = (|| -> Result<MappedBytes, StartError> {
                match piece {
                    Piece::Segment {
                        encoded,
                        source,
                        ...
                        let mut mapped = MappedBytes::identity(source, source_start)?;
                        if this.normalize_nfc {
                            mapped.normalize_nfc()?;
                        }
                        if this.add_prefix_space
                            && !mapped.bytes().is_empty()
                            && mapped.bytes()[0] != b' '
                        {
                            mapped.prepend_inserted(b" ", source_start)?;
                        }
                        if mapped.bytes() != encoded {
                            return Err(StartError::TokenBytesMismatch {
                                token_index: 0,
                                byte_offset: output.bytes().len(),
                            });
                        }
                        Ok(mapped)
                    }
                    Piece::Added { start, end, .. } => {
                        MappedBytes::identity(&input[start..end], start).map_err(Into::into)
                    }
                }
            })();
            match mapped {
                Ok(mapped) => output.append(mapped),
    ...
    #[cfg(feature = "profiling")]
    pub(crate) fn encode_with_added_tokens_profiled(
        &mut self,
        bytes: &[u8],
        out: &mut Vec<u32>,
        batch_sampling_phase: u64,
    ) -> ProfileSnapshot {
        let clock = ProfileClock::new();
        let started = clock.now_ms();
        let mut profile = ProfileAccumulator::default();
        profile.batch_sampling_phase = batch_sampling_phase;
        let pt = self.pretokenizer_type;
        let reserved_scan_ms =
            self.for_each_piece_profiled(bytes, &clock, |this, piece| match piece {
                Piece::Segment { encoded, .. } => this.memoized_encode_flat_profiled(
                    pt.pretokenize(encoded),
                    out,
                    &clock,
                    &mut profile,
                ),
        ...
        profile.reserved_scan_ms = reserved_scan_ms;
        profile.finish(clock.now_ms() - started)
    }

    #[cfg(feature = "profiling")]
    #[inline(never)]
    fn memoized_encode_flat_profiled<'i>(
        &mut self,
        ...
        loop {
            let sampled =
                sample_profile_batch(profile.pretoken_batches, profile.batch_sampling_phase);
            let fill_started = sampled.then(|| clock.now_ms());
            let cache = &self.pretoken_cache;
            let n = pretokens.fill_spans_keyed(&mut batch, &|h| cache.prefetch_l2(h));
            let fill_finished = fill_started.map(|started| {
                let finished = clock.now_ms();
                profile.pretokenize_ms += finished - started;
                finished
            });
            if n == 0 {
                break;
            }
            profile.pretoken_batches += 1;
            profile.pretokens += n as u64;
            let output_start = out.len();
            let misses_before = profile.short_cache_misses + profile.long_cache_misses;
            self.probe_emit_chunk_profiled(&batch, n, out, profile);
            let combined_ms = fill_finished.map(|started| clock.now_ms() - started);
            let misses_after = profile.short_cache_misses + profile.long_cache_misses;
            if let Some(combined_ms) = combined_ms {
                profile.sampled_pretoken_batches += 1;
                if misses_after == misses_before {
                    profile.output_assembly_ms += combined_ms;
                } else {
                    replay.clear();
                    let output_started = clock.now_ms();
                    self.probe_emit_chunk(&batch, n, &mut replay, |_, _| {});
                    let replay_ms = clock.now_ms() - output_started;
                    debug_assert_eq!(&out[output_start..], replay.as_slice());
                    let output_ms = replay_ms.min(combined_ms);
                    profile.output_assembly_ms += output_ms;
                    profile.merge_ms += combined_ms - output_ms;
                }
            }
            if n < PRETOKEN_CHUNK {
                break;
    ...
    #[cfg(feature = "profiling")]
    #[cfg_attr(feature = "profiling-sampling", inline(never))]
    #[cfg_attr(not(feature = "profiling-sampling"), inline(always))]
    fn probe_emit_chunk_profiled(
        &mut self,
        batch: &SpanBatch<'_>,
        n: usize,
        out: &mut Vec<u32>,
        ...
        for i in 0..n {
            ...
            let fast = found & (val & VAL_SPILL == 0) & (key != 0);
            let ab = ((val >> 8) & 0x00FF_FFFF) | (val & 0xFFFF_FFFF_0000_0000);
            // SAFETY: the reserve above leaves four output lanes per remaining entry.
            unsafe {
                let pointer = dst.add(w);
                (pointer as *mut u64).write_unaligned(ab);
                (pointer.add(2) as *mut u64).write_unaligned(ext);
            }
            w += if fast { (val & 0x7F) as usize } else { 0 };
            if !fast {
                // SAFETY: entry i was written by the current fill.
                let bytes = unsafe { batch.span(i) };
                w = self.probe_emit_slow_profiled(bytes, key, h, out, w, profile);
                dst = out.as_mut_ptr();
                table = self.pretoken_cache.probe_view();
            }
        }
        // SAFETY: every element below w was initialized by the fast stores or slow appends.
        unsafe { out.set_len(w) };
    }
    ...
    fn probe_emit_slow_profiled(
        ...
        // SAFETY: the profiled emit loop maintains the production cursor invariant.
        unsafe { out.set_len(w) };
        if key != 0 {
            match self.pretoken_cache.get_or_slot(key, h) {
                Ok((val, ext)) => {
                    ...
                    if val & VAL_SPILL == 0 {
                        ...
                        let start = (val >> 32) as usize;
                        // SAFETY: spill values were recorded from this arena.
                        let tokens = unsafe { self.token_arena.get_unchecked(start..start + len) };
                        out.extend_from_slice(token_ids_as_u32s(tokens));
                    }
                }
                Err(slot) => {
                    profile.short_cache_misses += 1;
                    self.encode_pretoken_miss_profiled(bytes, key, h, slot, out, &mut profile.rank);
                }
            }
        } else {
            match self.pretoken_cache_long.get(bytes) {
                Some(&(offset, len)) => {
                    let start = offset as usize;
                    // SAFETY: long-cache values were recorded from this arena.
                    let tokens =
                        unsafe { self.token_arena.get_unchecked(start..start + len as usize) };
                    out.extend_from_slice(token_ids_as_u32s(tokens));
                }
                None => {
                    profile.long_cache_misses += 1;
                    self.encode_pretoken_miss_profiled(bytes, 0, 0, 0, out, &mut profile.rank);
                }
            }
        }
        out.reserve(4 * PRETOKEN_CHUNK);
    ...
    fn merge_short_profiled(
        ...
        match byte_remapping {
            Some(br) => {
                for (dst, &byte) in buf[..n].iter_mut().zip(bytes) {
                    *dst = br.mapping[byte as usize];
                }
            }
            None => {
                for (dst, &byte) in buf[..n].iter_mut().zip(bytes) {
                    *dst = TokenId(byte as u32);
                }
            }
        }
        if n < 2 {
        ...
        let counters = RefCell::new(counters);
        match pair_ranks {
            Some(table) => bpe_merge_symbols_short_scalar(
                |a, b| table.rank_profiled(a, b, &mut counters.borrow_mut()),
                |a, b| table.prefetch_rank(a, b),
                buf,
                n,
            ),
            None => bpe_merge_symbols_short_scalar(
                |a, b| {
                    ...
                    counters.id_hash_lookups += 1;
                    merges.get(&(a, b)).map_or(u32::MAX, |merged| merged.0)
                },
                |_, _| {},
                buf,
                n,
    ...
    #[cfg(feature = "profiling")]
    fn seed_symbols_ranked_profiled(
        byte_remapping: Option<&ByteRemapping>,
        ranked_merges: &RankedMerges,
        ignore_merges: bool,
        vocab_inv: &HashMap<TokenBytes, TokenId, rustc_hash::FxBuildHasher>,
        ...
        let mut get = |a, b| {
            counters.explicit_rank_lookups += 1;
            ranked_merges.get(&super::ranked_merge_key(a, b)).copied()
        };
        bpe_merge_symbols_ranked_slice_by(&mut get, &mut buf[..n])
    }

    #[cfg(feature = "profiling")]
    fn encode_pretoken_miss_ranked_profiled(
        &mut self,
        bytes: &[u8],
        key: u128,
        h: u64,
        ...
        let ranked_merges = self
            .ranked_merges
            .clone()
            .expect("caller checked ranked_merges");
        if key != 0 {
            let mut buf = [TokenId(0); SHORT_MERGE_MAX];
            let n = Self::seed_symbols_ranked_profiled(
                self.byte_remapping.as_ref(),
                &ranked_merges,
                self.ignore_merges,
                &self.vocab_inv,
            ...
            && let Some(&id) = self.vocab_inv.get(bytes)
        {
            ...
            match self.byte_remapping.as_ref() {
                Some(br) => symbols.extend(bytes.iter().map(|&byte| br.mapping[byte as usize])),
                None => symbols.extend(bytes.iter().map(|&byte| TokenId::from(byte as u32))),
            }
            let mut get = |a, b| {
                counters.explicit_rank_lookups += 1;
                ranked_merges.get(&super::ranked_merge_key(a, b)).copied()
            };
            bpe_merge_symbols_ranked_by(&mut get, symbols);
        }
    ...
    #[cfg(feature = "profiling")]
    fn encode_pretoken_miss_profiled(
        &mut self,
        bytes: &[u8],
        key: u128,
        h: u64,
        ...
        if self.ranked_merges.is_some() {
            return self.encode_pretoken_miss_ranked_profiled(bytes, key, h, slot, out, counters);
        }
        if key != 0 {
            let mut buf = [TokenId(0); SHORT_MERGE_MAX];
            let n = if self.ignore_merges {
                if let Some(&id) = self.vocab_inv.get(bytes) {
                    buf[0] = id;
                    1
                } else {
                    Self::merge_short_profiled(
                        self.byte_remapping.as_ref(),
                        self.pair_ranks.as_deref(),
                        &self.merges,
                        bytes,
        ...
        let pair_ranks = self.pair_ranks.clone();
    ...
    pub(crate) fn decode_token_bytes(&self, id: u32) -> Option<&[u8]> {
        self.vocab
            .get(id as usize)
    ...
    pub fn cache_mem_stats(&self) -> (usize, usize, usize, usize, usize, usize, usize) {
        ...
        #[cfg(feature = "opt-miss-path-diet")]
        let long_key_bytes = self.pretoken_cache_long.key_bytes();
        (
...
mod profiling_tests {
    use super::*;
    use crate::bpe::ranked_merge_key;

    fn byte_vocab(extra: &[&[u8]]) -> Vec<Vec<u8>> {
        let mut vocab = (0..=255_u32)
            .map(|byte| vec![byte as u8])
            .collect::<Vec<_>>();
        vocab.extend(extra.iter().map(|bytes| bytes.to_vec()));
        vocab
    }

    fn id_ranked_tokenizer() -> Tokenizer {
        let mut merges = HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        merges.insert((TokenId(b'a' as u32), TokenId(b'b' as u32)), TokenId(256));
        Tokenizer::new(merges, byte_vocab(&[b"ab"]), None)
    }

    fn explicit_ranked_tokenizer() -> Tokenizer {
        let mut merges = HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        merges.insert(
            ranked_merge_key(TokenId(b'a' as u32), TokenId(b'b' as u32)),
            (TokenId(256), 1),
        );
        merges.insert(
            ranked_merge_key(TokenId(256), TokenId(b'c' as u32)),
            (TokenId(257), 0),
        );
        Tokenizer::new_ranked(merges, byte_vocab(&[b"ab", b"abc"]), None)
    }

    fn assert_profile_matches(
        mut plain: Tokenizer,
        mut profiled: Tokenizer,
        input: &[u8],
    ) -> ProfileSnapshot {
        let mut expected = Vec::new();
        plain.encode_with_added_tokens_flat(input, &mut expected);
        let mut actual = Vec::new();
        let profile = profiled.encode_with_added_tokens_profiled(input, &mut actual, 0);
        assert_eq!(actual, expected);
        assert_eq!(profile.schema_version, 1);
        assert!(profile.internal_ms >= 0.0);
        assert!(profile.pretoken_batches > 0);
        assert!(profile.pretokens > 0);
        let children = profile.reserved_scan_ms
            + profile.pretokenize_ms
            + profile.merge_ms
            + profile.output_assembly_ms;
        assert!((children - profile.internal_ms).abs() < 0.001);
        profile
    }

    #[test]
    fn profiled_id_ranked_path_is_exact_and_counted() {
        let profile =
            assert_profile_matches(id_ranked_tokenizer(), id_ranked_tokenizer(), b"ababx ababz");
        assert!(profile.short_cache_misses > 0);
        assert!(profile.rank.id_rank_lookups > 0);
        assert!(profile.rank.dense_lookups > 0);
        assert_eq!(profile.rank.explicit_rank_lookups, 0);
    }

    #[test]
    fn profiled_explicit_rank_path_is_exact_and_counted() {
        let profile = assert_profile_matches(
            explicit_ranked_tokenizer(),
            explicit_ranked_tokenizer(),
            b"abcx abcy",
        );
        assert!(profile.short_cache_misses > 0);
        assert!(profile.rank.explicit_rank_lookups > 0);
        assert_eq!(profile.rank.id_rank_lookups, 0);
    }

    #[test]
    fn profiled_long_and_added_paths_are_exact() {
        let mut plain = id_ranked_tokenizer();
        let mut profiled = id_ranked_tokenizer();
        let added = AddedTokenDef {
            content: Arc::from(b"<special>".as_slice()),
            id: TokenId(300),
            lstrip: false,
            rstrip: false,
        };
        plain.set_added_tokens(vec![added.clone()]);
        profiled.set_added_tokens(vec![added]);
        let profile = assert_profile_matches(plain, profiled, b"abcdefghijklmnopqr<special>ababx");
        assert!(profile.long_cache_misses > 0);
        assert!(profile.reserved_scan_ms >= 0.0);
    }
}
...
mod test_util {
    ...
    pub(super) fn byte_tokenizer() -> Tokenizer {
...
mod tests {
    ...
    fn byte_only_tokenizer() -> Tokenizer {
        let vocab = (0..=255_u32)
            .map(|byte| vec![byte as u8])
            .collect::<Vec<_>>();
        Tokenizer::from_ranks(vocab).unwrap()
    }

    #[test]
    fn token_starts_cover_prefix_trim_and_stream_validation() {
        let mut tokenizer = byte_only_tokenizer();
        tokenizer.set_add_prefix_space(true);
        tokenizer.set_trim_offsets(true);
        let mut ids = Vec::new();
        tokenizer.encode_with_added_tokens_flat(b"a", &mut ids);
        assert_eq!(ids, [u32::from(b' '), u32::from(b'a')]);
        assert_eq!(tokenizer.token_starts(b"a", &ids).unwrap(), [0, 0]);

        let mut trimmed = byte_only_tokenizer();
        trimmed.set_trim_offsets(true);
        let mut ids = Vec::new();
        trimmed.encode_with_added_tokens_flat(b"  a", &mut ids);
        assert_eq!(
            trimmed.identity_token_starts(b"  a", &ids).unwrap(),
            Some(vec![1, 2, 2])
        );
        assert_eq!(trimmed.token_starts(b"  a", &ids).unwrap(), [1, 2, 2]);
        assert!(matches!(
            trimmed.token_starts(b"  a", &[u32::from(b'a')]),
            Err(StartError::TokenBytesMismatch { .. })
                | Err(StartError::TransformedLengthMismatch { .. })
        ));
        assert_eq!(
            trimmed.token_starts(b"", &[999]).unwrap_err(),
            StartError::UnknownTokenId(999)
        );
    }

    #[test]
    fn token_starts_cover_added_token_insertions_and_removals() {
        let mut tokenizer = byte_only_tokenizer();
        tokenizer.add_special_token(b"<x>".to_vec(), TokenId::from(300));
        ...
        assert_eq!(ids, [u32::from(b'a'), 300, u32::from(b'b')]);
        assert_eq!(tokenizer.token_starts(input, &ids).unwrap(), [0, 3, 8]);
    }

    #[test]
    fn token_starts_round_changed_nfc_clusters_to_the_source_start() {
        let mut tokenizer = byte_only_tokenizer();
        tokenizer.set_normalize_nfc(true);
        let input = "A\u{30a}z".as_bytes();
        let mut ids = Vec::new();
        tokenizer.encode_with_added_tokens_flat(input, &mut ids);
        assert_eq!(ids, [0xc3, 0x85, u32::from(b'z')]);
        assert_eq!(tokenizer.identity_token_starts(input, &ids).unwrap(), None);
        assert_eq!(tokenizer.token_starts(input, &ids).unwrap(), [0, 0, 3]);

        let input = "Åz".as_bytes();
        ids.clear();
        tokenizer.encode_with_added_tokens_flat(input, &mut ids);
        assert_eq!(
            tokenizer.identity_token_starts(input, &ids).unwrap(),
            Some(vec![0, 1, 2])
        );
    }
    ...
    #[cfg(feature = "opt-resident-diet")]
    #[test]
    fn discarded_id_merges_preserve_encode_fork_and_introspection() {
        let mut merges: HashMap<(TokenId, TokenId), TokenId, rustc_hash::FxBuildHasher> =
            HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        merges.insert((TokenId(b'a' as u32), TokenId(b'b' as u32)), TokenId(256));
        let mut vocab: Vec<Vec<u8>> = (0..=255u32).map(|byte| vec![byte as u8]).collect();
        vocab.push(b"ab".to_vec());
        let mut tokenizer = Tokenizer::new(merges, vocab, None);
        let entries_before: Vec<(Vec<u8>, Vec<u8>)> = tokenizer
            .merge_entries()
            .into_iter()
            .map(|(left, right)| (left.to_vec(), right.to_vec()))
            .collect();

        assert!(tokenizer.discard_redundant_id_merges());
        assert!(tokenizer.merges.is_empty());
        let entries_after: Vec<(Vec<u8>, Vec<u8>)> = tokenizer
            .merge_entries()
            .into_iter()
            .map(|(left, right)| (left.to_vec(), right.to_vec()))
            .collect();
        ...
        let encode = |tokenizer: &mut Tokenizer| {
            let mut output = Vec::new();
            tokenizer.memoized_encode(crate::pretokenize::pretokenize_as_iter(b"ab ab"), |ids| {
                output.extend_from_slice(ids)
            });
            output
        };
        let mut fork = tokenizer.fork();
        assert_eq!(encode(&mut tokenizer), encode(&mut fork));
        assert!(fork.merges.is_empty());
    }
    ...
    /// A merge-reachable byte vocabulary must use the pair-rank table and
    /// seed each short vocabulary word with its own ID.
    #[cfg(not(feature = "opt-cold-diet"))]
    #[test]
    fn pair_rank_table_and_vocab_seed() {
        use crate::pretokenize::{pack_pretoken_key, pretoken_key_hash};
        let mut merges = HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        merges.insert((TokenId(97), TokenId(98)), TokenId(256));
        merges.insert((TokenId(256), TokenId(99)), TokenId(257));
        let mut vocab: Vec<Vec<u8>> = (0..=u8::MAX).map(|byte| vec![byte]).collect();
        vocab.push(b"ab".to_vec());
        vocab.push(b"abc".to_vec());
        let tokenizer = Tokenizer::new(merges, vocab, None);

        let table = tokenizer
            .pair_ranks
            .as_deref()
            .expect("merge-reachable vocabulary must take the pair-rank fast path");
        ...
        // the map too.
        for a in (0..258u32).step_by(17) {
            for b in (0..258u32).step_by(19) {
    ...
    #[cfg(feature = "opt-cold-diet")]
    #[test]
    fn lazy_seed_preserves_merge_and_ignore_merges_results() {
        use crate::pretokenize::SpanIter;

        let encode = |tokenizer: &mut Tokenizer, input: &[u8]| {
            let mut output = Vec::new();
            tokenizer.memoized_encode(SpanIter([Pretoken(input)].into_iter()), |tokens| {
                output.extend(tokens.iter().map(|token| token.0));
            });
            output
        };

        let mut merges = HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        merges.insert((TokenId(104), TokenId(105)), TokenId(256));
        let mut vocab: Vec<Vec<u8>> = (0..=255u32).map(|byte| vec![byte as u8]).collect();
        vocab.push(b"hi".to_vec());
        let mut merged = Tokenizer::new(merges, vocab.clone(), None);
        assert_eq!(merged.pretoken_cache.len(), 0);
        assert_eq!(encode(&mut merged, b"hi"), [256]);
        assert_eq!(encode(&mut merged, b"hi"), [256]);
        assert_eq!(merged.pretoken_cache.len(), 1);
        assert_eq!(merged.fork().pretoken_cache.len(), 0);

        let mut ignored = Tokenizer::new(HashMap::default(), vocab, None);
        ignored.set_ignore_merges(true);
        assert_eq!(ignored.pretoken_cache.len(), 0);
        assert_eq!(encode(&mut ignored, b"hi"), [256]);
        assert_eq!(encode(&mut ignored, b"hi"), [256]);
        assert_eq!(ignored.pretoken_cache.len(), 1);
    }
...
#[cfg(test)]
mod verify_heavy {
    use super::test_util::{XorShift64, byte_tokenizer, plain_encode_pretoken};
    use super::*;
    use std::io::Read;
    ...
    fn verify_memoized_encode_matches_reference_owt_50m() {
        let mut tokenizer = byte_tokenizer();
    ...
    /// A merge-unreachable vocabulary entry must be cached as its byte
    /// decomposition instead of its own vocabulary ID.
    #[test]
    fn verify_vocab_seeded_cache_matches_merge_decomposition() {
        let merges = HashMap::with_hasher(rustc_hash::FxBuildHasher {});
        let mut vocab: Vec<Vec<u8>> = (0..=u8::MAX).map(|byte| vec![byte]).collect();
        vocab.push(b"abc".to_vec());
        vocab.push(b"abcdefghijklmnop".to_vec());
        let mut tok = Tokenizer::new(merges, vocab, None);

        let mut cached: Vec<u32> = Vec::new();
        tok.encode_with_added_tokens_flat(b"abc", &mut cached);
        assert_eq!(
            cached,
            vec![97, 98, 99],
        ...
        let long_entry = b"abcdefghijklmnop";
        let mut cached_long: Vec<u32> = Vec::new();
        tok.encode_with_added_tokens_flat(long_entry, &mut cached_long);
        let expected: Vec<u32> = long_entry.iter().map(|&byte| u32::from(byte)).collect();
        assert_eq!(cached_long, expected);
    }
...
mod verify_alignment {
    use super::test_util::{XorShift64, byte_tokenizer, two_phase_lens};
    use super::*;
...
#[cfg(test)]
mod walker_edge {
    use super::test_util::{XorShift64, byte_tokenizer, plain_encode_pretoken, two_phase_lens};
    use super::*;
