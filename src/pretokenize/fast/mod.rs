...
pub(crate) fn classify_o200k_masks(bytes: &[u8]) -> u64 {
    use mask::MaskScheme;
    let mut accumulated = 0_u64;
    let mut offset = 0_usize;
    while offset + 64 <= bytes.len() {
        let (usable, bad) = <o200k::O200kScheme as MaskScheme>::batch_masks(bytes, offset);
        accumulated = accumulated.wrapping_add(usable ^ bad);
        offset += 64;
    }
    accumulated
}
