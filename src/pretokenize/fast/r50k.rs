...
fn batch_masks(bytes: &[u8], scan: usize) -> (u64, u64) {
    ...
    let am = super::wasm_relaxed_simd::ascii_masks(bytes, scan).ascii;
    #[cfg(not(all(feature = "opt-relaxed-simd", target_feature = "relaxed-simd")))]
    let am = super::wasm_simd128::ascii_masks(bytes, scan);
    let whitespace = am.s | am.wt | am.n;
    if am.hi != 0 {
        return extended_masks(bytes, scan, am.l, am.d, am.s, whitespace, am.hi, am.ap);
    }
    ascii_batch_algebra(bytes, scan, am.l, am.d, am.s, whitespace, am.ap)
}
