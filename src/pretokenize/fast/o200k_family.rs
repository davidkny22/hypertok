...
pub(crate) fn batch_masks<
    ...
    const SLASH: bool,
    const HAN: bool,
>(bytes: &[u8], scan: usize) -> (u64, u64) {
    if scan + 70 > bytes.len() {
        return (0, u64::MAX);
    }
    #[cfg(all(feature = "opt-relaxed-simd", target_feature = "relaxed-simd"))]
    let (am, ax) = {
        let masks = super::wasm_relaxed_simd::ascii_masks(bytes, scan);
        (
            masks.ascii,
            OAsciiExtra {
                up: masks.up,
    ...
    #[cfg(not(all(feature = "opt-relaxed-simd", target_feature = "relaxed-simd")))]
    let (am, ax) = (
        super::wasm_simd128::ascii_masks(bytes, scan),
        OAsciiExtra {
            up: super::wasm_simd128::byte_range_mask(bytes, scan, b'A', 25),
            sl: super::wasm_simd128::byte_eq_mask(bytes, scan, b'/'),
        },
    );
    if am.hi != 0
        || (scan >= 1 && bytes[scan - 1] >= 0x80)
