...
#[cfg(all(target_arch = "x86_64", not(feature = "scalar-pretokenizer")))]
#[inline]
pub(crate) fn simd_scanner_available() -> bool {
    avx512_scanner_available() || avx2_scanner_available()
}

#[cfg(any(not(target_arch = "x86_64"), feature = "scalar-pretokenizer"))]
#[inline]
pub(crate) fn simd_scanner_available() -> bool {
    #[cfg(all(
    ...
    if super::level_select::scalar_scanner_forced() {
        return false;
    }
    !cfg!(feature = "scalar-pretokenizer")
...
unsafe fn batch_masks_dyn_avx512<S: MaskScheme>(bytes: &[u8], scan: usize) -> (u64, u64) {
    ...
    if super::block_dispatch::prefer_scalar(bytes, scan) {
        return (0, u64::MAX);
    }
...
#[cfg(target_arch = "x86_64")]
#[inline(always)]
unsafe fn batch_masks_pinned_x86<S: MaskScheme, const AVX512: bool>(
    bytes: &[u8],
    scan: usize,
) -> (u64, u64) {
    #[cfg(feature = "opt-block-dispatch")]
    ...
    // SAFETY: the caller selected a matching runtime-detected tier.
...
#[inline(always)]
fn batch_masks_dispatched<S: MaskScheme>(bytes: &[u8], scan: usize) -> (u64, u64) {
    #[cfg(all(feature = "opt-block-dispatch", not(target_arch = "x86_64")))]
    if super::block_dispatch::prefer_scalar(bytes, scan) {
        return (0, u64::MAX);
    }
...
impl MaskState {
    ...
    #[cfg(any(target_arch = "aarch64", target_arch = "x86_64", all(target_arch = "wasm32", target_feature = "simd128")))]
    #[inline(always)]
        ...
        loop {
            ...
            #[cfg(any(target_arch = "aarch64", target_arch = "x86_64", all(target_arch = "wasm32", target_feature = "simd128")))]
            {
                ...
                let (usable, bad) = if self.pre_base == self.scan {
                    ...
                    batch_masks_dispatched::<S>(bytes, self.scan)
                };
                ...
                if self.scan + 64 <= len {
                    let (u2, b2) = batch_masks_dispatched::<S>(bytes, self.scan);
            ...
            #[cfg(not(any(target_arch = "aarch64", target_arch = "x86_64", all(target_arch = "wasm32", target_feature = "simd128"))))]
            {
            ...
            'harvest: while nb < needed {
                ...
                let (usable, bad) = match X86_TIER {
                    ...
                    X86_TIER_AVX512 | X86_TIER_AVX512_VBMI2 => unsafe {
                        batch_masks_pinned_x86::<S, true>(bytes, base)
                    },
                    X86_TIER_AVX2 => unsafe { batch_masks_pinned_x86::<S, false>(bytes, base) },
                    _ => batch_masks_dispatched::<S>(bytes, base),
                };
                #[cfg(not(target_arch = "x86_64"))]
                let (usable, bad) = batch_masks_dispatched::<S>(bytes, base);
