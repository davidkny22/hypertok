...
/// one [`SpanBatch`] fill.
#[cfg(not(feature = "opt-batch-locality"))]
pub const PRETOKEN_CHUNK: usize = 256;
#[cfg(feature = "opt-batch-locality")]
pub const PRETOKEN_CHUNK: usize = 128;
...
compile_error!("hypertok's key packing and token-lane stores assume little-endian byte order");
...
mod span_source_tests {
    ...
    #[cfg(feature = "opt-batch-locality")]
    #[test]
    fn locality_batch_uses_half_the_default_entries() {
        assert_eq!(PRETOKEN_CHUNK, 128);
        assert_eq!(std::mem::size_of::<BatchEntry>(), 32);
        assert_eq!(
            std::mem::size_of::<SpanBatch<'_>>(),
            (128 + SPAN_BATCH_SLACK) * 32
        );
    }
